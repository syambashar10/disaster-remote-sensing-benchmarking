...
def validate_yolo_segmentation_line(line: str):
...
def main():
    output_dir = Path("outputs/yolo_segmentation/xbd_sample")
    ...
    for label_file in label_files:
        ...
        for line in lines:
            validate_yolo_segmentation_line(line)

    assert total_lines == result["label_lines_exported"]

    verification_output = {
        "test_name": "xbd_yolo_segmentation_exporter_test",
        "status": "passed",
        "result": result,
        "total_label_lines_checked": total_lines,
    }

    verification_path = Path("outputs/verification/xbd_yolo_segmentation_exporter_test.json")
    verification_path.parent.mkdir(parents=True, exist_ok=True)

    with verification_path.open("w", encoding="utf-8") as f:
        json.dump(verification_output, f, indent=2)

    print("xBD YOLO segmentation exporter test passed.")
